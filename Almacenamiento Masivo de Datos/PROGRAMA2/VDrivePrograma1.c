...
void put_enter(){
   putc(0x0D);
}          //lista comandos de configuracion del vdrive
...
{
          usb_init_cs();            // SE HABILITA LA COMUNICACION USB
          lcd_init();               // INICIALIZA LCD 
          Set_Char(0,new_car);
inicio:   PUT_SCS();                //SE CONFIGURA AL VDRIVE PARA COMANDS CORTOS
          PUT_IPH();                //SE CONFIGURA PARA QUE ACEPTE DATOS BINARIOS
         ObtenerDato();             //SE CAPTURA SUCESO
                   
          if (act_lcd)              //SI HAY RESPUESTA DEL VDRIVE
          {                         // SE IMPRIME VDRIVE CONECTADO
              printf(lcd_putc,"\f     VDRIVE");
              printf(lcd_putc,"\n   CONECTADO");
              act_lcd=0;
              ON(PIN_C1);
              DELAY_ms(500);
          ...
          else                           // SI NO SE OBTIENE RESPUESTA DEL VDRIVE SE ASUME        
          {                               // QUE NO SE ENCUENTRA CONECTADO 
             if(band1) goto inicio;
             else{printf(LCD_PUTC,"\fNO SE ENCONTRO");printf(LCD_PUTC,"\n     VDRIVE");band1=1;OFF(PIN_C1); goto inicio;}
          }   
            
         WHILE(TRUE)         //BUCLE PRINCIPAL
         {
             PUT_FWV();       //SE SOLICITA LA VERSION DEL FIRMWARE
             ObtenerDato();      //SE CAPTURA LA RESPUESTA DEL VDRIVE
             display_Dato(DATO,300); //SE DESPLEGA EL DATO
             delay_ms(300);                        
             
             PUT_RD_FILE();     //SE MANDA COMANDO  DESPLEGAR TODO EL CONTENIDO
             obtenerDato();      //
             display_dato(DATO,700);// SE DESPLE
             DELAY_ms(500);     
             IF ((!INPUT(PIN_A4))&&(!INPUT(PIN_A2))) {menu_config();}  // CONDION PARA ENTRAR AL VDRIVE 
