...
*----------------------------------------------------------------}
procedure TFrmUDPMain.SendActExecute(Sender: TObject);
begin
...
*-----------------------------------------------------------------}
procedure TFrmUDPMain.ConnectActExecute(Sender: TObject);
var
  add: TSockAddr;
begin
...
*-----------------------------------------------------------------------}
procedure TFrmUDPMain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  IdUDPServer1.Active := FALSE;
  IdUDPClient1.active := FALSE;
end;
...
*------------------------------------------------------------------------}
procedure TFrmUDPMain.IdUDPServer1UDPException(Sender: TObject;
  ABinding: TIdSocketHandle; const AMessage: string;
  const AExceptionClass: TClass);
begin
  if not FportClosing
...
*------------------------------------------------------------------------}
procedure TFrmUDPMain.IdUDPServer1UDPRead(Sender: TObject; AData: TBytes;
  ABinding: TIdSocketHandle);
var
  index: integer;
  Astr: string;
begin
  ...
  Astr := Abinding.PeerIP + ':' + inttostr(idUDPServer1.DefaultPort) + ' -> ' +
          Astr;
...
*-------------------------------------------------------------------------}
procedure TFrmUDPMain.ConnectActUpdate(Sender: TObject);
const
...
*-------------------------------------------------------------------------}
procedure TFrmUDPMain.SendActUpdate(Sender: TObject);
begin
